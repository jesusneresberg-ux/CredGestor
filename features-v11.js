...
(function(){
  ...
  async function loginGoogleV11(){
    ...
    loginTokenClient=google.accounts.oauth2.initTokenClient({client_id:clientId,scope:'openid email profile https://www.googleapis.com/auth/drive.file',prompt:'select_account',callback:async resp=>{
      ...
      try{
        ...
        const profile={email:p.email||'',name:p.name||p.email||'Conta Google',picture:p.picture||'',sub:p.sub||'',at:Date.now()};
        setSession(profile);
        window.__credigestorGoogleAccessV12={token:resp.access_token,expiresAt:Date.now()+(Number(resp.expires_in)||3600)*1000,profile};
  ...
  function injectAccountSettingsV11(){
    ...
    const logout=document.getElementById('logoutGoogleV11');if(logout)logout.onclick=()=>{const access=window.__credigestorGoogleAccessV12;window.__credigestorGoogleAccessV12=null;if(access?.token&&window.google?.accounts?.oauth2?.revoke)google.accounts.oauth2.revoke(access.token,()=>{});window.dispatchEvent(new CustomEvent('credigestor:google-logout'));clearSession();renderSettings();if(state.settings.requireGoogleLoginV11)showLoginOverlayV11()};
    document.querySelectorAll('.card .note').forEach(n=>{if(n.textContent.includes('próximo passo é migrar para banco em nuvem com login'))n.textContent='Os dados permanecem disponíveis neste navegador e podem ser sincronizados com a nuvem usando sua Conta Google.'});
  }
